...
class Tacotron2(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # variable length reference embedding in length of text
    ...
    def inference(self, inputs):
        ...
        if hasattr(self, 'gst'):
            if isinstance(style_input, int): # I don't know how to optimize this query vector for no reference case
                ...
                # variable length residual embedding
                # F0 as input for residual encoder!
                # f0s *= 0.5
                embedded_gst = self.gst(f0s, embedded_text)
                numpy_gst = embedded_gst.cpu().numpy()
                embedded_gst = torch.from_numpy(numpy_gst).cuda()
